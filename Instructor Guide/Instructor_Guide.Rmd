...
- `ggplot2`, which makes nicer-looking graphs than base R and which you may want to use to make graphs for class
- Causal diagrams
...
Once you have your DAG object, run it through `tidy_dagitty()`, and then finally plot it using `ggdag()`, as below.
...
The `ggplot2` package (available on its own, or, like `dplyr`, it's also part of the `tidyverse`) is probably the best data-viz tool on the market. In my opinion it's just a shade too hard to use to force the students to use it, but when you're making your own plots you might want to consider making them look that extra shade nicer.
